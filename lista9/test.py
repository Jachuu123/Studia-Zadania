...
def print_board(board):
    ...
    for row in board:
        print(" ".join(
            f"{symbols[cell[0]]}{cell[1] if cell and cell[0] else ' '}".ljust(2) if cell else " .".ljust(2) for cell in row
        ))
...
def resolve_battle(attacker, defender):
    ...
    rules = {"K": "P", "N": "K", "P": "N"}  # Co wygrywa z czym
...
def update_board(board):
    ...
    new_board = [[cell if cell else None for cell in row] for row in board]  # Głęboka kopia planszy

    for y in range(height):
        for x in range(width):
            if board[y][x] is None:  # Pomijamy puste pola
                continue
            ...
            if neighbor is None:
                ...
                if strength > 1:
                    ...
                    #print(f"Cell ({x}, {y}) ({cell_type}{strength}) populated ({nx}, {ny}) with ({cell_type}{strength - 1}).")
            elif neighbor[0] == cell_type:
                # Ten sam typ - nic się nie dzieje
                continue
            elif neighbor[0] is not None:  # Sprawdzamy, czy neighbor ma prawidłowy typ
                # Pojedynek
                new_cell, new_neighbor = resolve_battle((cell_type, strength), neighbor)
                #print(f"Battle: ({x}, {y}) ({cell_type}{strength}) vs ({nx}, {ny}) ({neighbor[0]}{neighbor[1]}) -> "
                #      f"Result: {new_cell} and {new_neighbor}")
                new_board[y][x] = new_cell if new_cell[1] > 0 else None
